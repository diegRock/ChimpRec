...
class raw_tracking_data_reader():
    ...
    def read(self):
        # Parse line-by-line so that empty frames are preserved as empty blocks instead
        # of being collapsed away by a naive split on "#\n". Dropping empty
        # frames silently shifts every subsequent bbox in time and produces
        # a growing offset between bboxes and the chimps in the preview.
        parsed_content = []
        current_block = None
            ...
            for raw_line in text_file:
                ...
                if line == "#":
                    if current_block is not None:
                        parsed_content.append(current_block)
                    current_block = []
                    continue
                if current_block is None:
                    # Stray content before the first '#' -- skip.
                    continue
                if line == "":
                    # Padding line used by data_writer for empty frames.
                    continue
        ...
        while ret:
            ...
            for id_bbox in bboxes:
                ...
                if annotation_type == "bbox":
                    draw_bbox(frame, colors_used[name], bbox, f"ID: {name}")
                elif annotation_type == "triangle":
                    draw_triangle(frame, colors_used[name], bbox, name)

            if draw_frame_count:
                label_text = f"ChimpRec - Auto-Label vFinal  |  {frame_number}"
                font_scale = 0.65
                thickness = 1
                (w, h), _ = cv2.getTextSize(label_text, cv2.FONT_HERSHEY_COMPLEX, font_scale, thickness)
                x, y = 10, h + 10
                overlay = frame.copy()
                cv2.rectangle(overlay, (x - 5, y - h - 5), (x + w + 5, y + 5), (57, 46, 135), -1)
                cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
                cv2.putText(frame, label_text, (x, y), cv2.FONT_HERSHEY_COMPLEX, font_scale, (255, 255, 255), thickness)
