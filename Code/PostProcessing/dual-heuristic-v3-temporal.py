...
def smart_stitch_global_automated(tracker_file, video_file, output_file, weights_path):
    """
    The new main function that includes:
    1. Ghost Removal (Short tracks kept separate)
    2. Physical Constraints (Prevents ID 1 appearing twice in same frame)
    """
    # Init Model
    ...
    # 2. Extract Representative Embeddings
    ...
    if len(track_embeddings) < 2:
        print("Not enough tracks to cluster. Exiting.")
        return

    # 3. Compute Base Distance Matrix (Visual Only)
    sim_matrix = np.dot(track_embeddings, track_embeddings.T) 
    dist_matrix = 1.0 - sim_matrix
    dist_matrix[dist_matrix < 0] = 0 
    
    # --- 3b. APPLY PHYSICAL CONSTRAINTS (THE FIX) ---
    print("🛡️ Applying temporal constraints (Physics Penalty)...")
    penalty_count = 0
    
    # We iterate through the matrix and check for temporal overlaps
    num_valid = len(valid_track_indices)
    
    # Create sets of frames for fast lookup
    track_frames_sets = []
    for idx in valid_track_indices:
        track_frames_sets.append(set(long_tracks[idx]['frames']))

    for i in range(num_valid):
        for j in range(i + 1, num_valid):
            # Check overlap using sets
            if not track_frames_sets[i].isdisjoint(track_frames_sets[j]):
                # THEY OVERLAP! Force distance to max.
                # Even if they look identical (dist=0.01), we make them dist=100.0
                dist_matrix[i, j] = 100.0 
                dist_matrix[j, i] = 100.0
                penalty_count += 1
                
    print(f"   -> Applied physics penalty to {penalty_count} impossible pairs.")

    # 4. AUTOMATED K-ESTIMATION (Using Constrained Matrix)
    # We pass the constrained matrix so silhouette score knows they are 'far apart'
    ...
    # 5. Clustering
    # We use 'complete' linkage (or average). 
    # 'average' is fine here because 100.0 is so huge it will drag the average up massively.
        ...
        for i in range(total_frames + 1):
            ...
            if i in frame_data:
                for line in frame_data[i]:
                    f.write(line + "\n")
    print(f"💾 Saved constraint-aware tracks to {output_file}")
