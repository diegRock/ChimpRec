...
def smart_stitch_global_automated(tracker_file, video_file, output_file, weights_path):
    ...
    # Filter out tiny tracks (noise) to speed up clustering and avoid false positives
    long_tracks = [t for t in track_list if len(t['frames']) > 30]
    short_tracks = [t for t in track_list if len(t['frames']) <= 30]
    print(f"🔍 Extracted {len(long_tracks)} significant tracks.")
    ...
    # --- 3. THE NEW AUTOMATED K-ESTIMATION ---
    # We pass the valid tracks so it can calculate concurrent physical overlaps
    ...
    # 4. Clustering with the newly discovered K
    ...
    # Safely map short/unclustered tracks
...
if __name__ == "__main__":
    VIDEO_PATH = "../Tracking/Manual Correction/input/20241002 - 12h28.MP4"
    TRACKER_INPUT = "../Tracking/Manual Correction/output/temp/raw_output/20241002 - 12h28_v3_bytetrack.txt"
    TRACKER_OUTPUT = "../Tracking/Manual Correction/output/temp/raw_output/20241002 - 12h28_v6_bytetrack_auto_clustered.txt"
