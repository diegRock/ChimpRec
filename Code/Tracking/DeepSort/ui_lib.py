        ...
        with open(self.text_file_path, 'r') as text_file:
            for line in text_file.read().split("\n"):
                ...
                if ":" in line:  # name present
                    ...
                    # Normalize spaces around commas: "1, 2, 3" -> "1,2,3"
                    # This ensures split(" ") keeps the group "1,2,3*10-100" together as one token
                    rhs = re.sub(r'\s*,\s*', ',', rhs)
                    
                    tokens = rhs.split(" ")
                    for tok in tokens:
                        if not tok: continue # Skip empty tokens resulting from double spaces
...
def perform_tracking(input_video_path, output_text_file_path, detection_model, tracker, confidence_threshold, model_feature_extraction):
    ...
    Saves the bounding boxes metadata within <output_text_file_path>
    """
    cap = cv2.VideoCapture(input_video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    ret, frame = cap.read()
